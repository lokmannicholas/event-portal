...
function buildEventPaths(record: AnyRecord) {
  ...
  const publicUrl = `/p/${partitionCode}/e/${eventCode}`;
  const publicBaseUrl = record.publicBaseUrl ? String(record.publicBaseUrl).replace(/\/$/, '') : 'https://erp.local';

  return {
    eventCode,
    partitionCode,
    publicUrl,
    qrPayload: publicBaseUrl.startsWith('http') ? `${publicBaseUrl}${publicUrl}` : `${publicBaseUrl}${publicUrl}`,
  };
}
...
function mapEventDetail(record: AnyRecord): EventDetailDTO {
  return {
    event: {
      ...
      notifications: toArray<any>(record.notificationTemplates).map((notification) => ({
