...
type EventNoticeType = 'REGISTRATION' | 'ANNOUNCEMENT' | 'EVENT_UPDATE';
...
function getDefaultEventNotifications() {
  return [];
}

function buildEventNoticeTemplates(formData: FormData) {
  const definitions: Array<{ type: EventNoticeType; fieldName: string }> = [
    { type: 'REGISTRATION', fieldName: 'registrationNoticeTemplateDocumentId' },
    { type: 'ANNOUNCEMENT', fieldName: 'announcementNoticeTemplateDocumentId' },
    { type: 'EVENT_UPDATE', fieldName: 'eventUpdateNoticeTemplateDocumentId' },
  ];

  return definitions
    .map((definition) => {
      const noticeTemplateDocumentId = getOptionalValue(formData, definition.fieldName);

      if (!noticeTemplateDocumentId) {
        return undefined;
      }

      return {
        templateType: definition.type,
        noticeTemplate: { documentId: noticeTemplateDocumentId },
        enabled: true,
      };
    })
  ...
  switch (kind) {
    ...
    case 'event': {
      ...
      const payload = {
        ...
        eventStatus: getValue(formData, 'status') as EventStatus,
        ...
        notificationTemplates: buildEventNoticeTemplates(formData) ?? getDefaultEventNotifications(),
        eventSlots,
      };
  ...
  switch (kind) {
    ...
    case 'event': {
      ...
      const payload = {
        ...
        userPartition: partitionDocumentId,
        template: templateRecord?.documentId,
        notificationTemplates: buildEventNoticeTemplates(formData),
    ...
    const duplicateResponse = await eventPortalSdk.events.create(
      {
        ...
        notificationTemplates: Array.isArray(source.notificationTemplates)
          ? source.notificationTemplates.map((notification) => ({
              templateType: notification.templateType,
              noticeTemplate: getRelationDocumentId(notification.noticeTemplate)
                ? { documentId: getRelationDocumentId(notification.noticeTemplate)! }
                : null,
