...
export default async function Page({ searchParams }: PageProps) {
  ...
  const copy = {
    ...
    subtitle: getPortalText(language, 'Reusable form blueprints that define registration fields and partition coverage for the registration experience.', '定義登記欄位及分區覆蓋範圍的可重用表單藍圖。'),
    createTemplate: getPortalText(language, 'Create template', '建立模板'),
    cardTitle: getPortalText(language, 'Templates', '模板'),
    cardDescription: getPortalText(language, 'Each template controls field selection and partition coverage for event registration forms. Open a record to view or update it.', '每個模板控制活動登記表單的欄位選擇及分區覆蓋範圍。開啟記錄以檢視或更新。'),
    template: getPortalText(language, 'Template', '模板'),
    partitions: getPortalText(language, 'Partitions', '分區'),
    ...
    selectedTemplateDescription: getPortalText(language, 'Sample field library and partition coverage from the first template.', '從第一個模板顯示其欄位庫及分區覆蓋範圍範例。'),
            ...
            <Card title={copy.cardTitle} description={copy.cardDescription}>
              <SimpleTable
                columns={[
                  ...
                  { key: 'partitions', label: copy.partitions },
                ...
                <Stack gap={14}>
                  <KeyValueList
                    items={[
                      ...
                      { label: copy.partitions, value: primaryTemplate.partitionCodes.join(', ') || '-' },
