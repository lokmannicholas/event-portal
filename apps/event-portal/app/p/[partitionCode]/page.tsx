...
export default async function Page({ params }: PageProps) {
  ...
  const headerCaption = landing?.partition.groupCompanyName ?? landing?.events[0]?.companyName;

  if (!landing || landing.events.length === 0) {
    return (
      <ErpShell
        title="Vaccination Event Master"
        subtitle="No accessible event could be found for the selected partition."
        partitionCode={partitionCode}
        headerCaption={headerCaption}
      >
        <EmptyState
          title="No accessible event"
          description="There is currently no event visible for this partition under the registration, event-period, and expired display rules. Check the QR code or contact QHMS support."
        />
      </ErpShell>
    );
  }
